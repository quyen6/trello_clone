 ...
 */
function CardDescriptionMdEditor({ cardDescProp, handleUpdateCardDesc }) {
  ...
  const [cardDescription, setCardDescription] = useState(cardDescProp);

  const updateCardDescription = () => {
    setMarkdownEditMode(false);
    handleUpdateCardDesc(cardDescription);
  };
    ...
    <Box sx={{ mt: -4 }}>
      {markdownEditMode ? (
        <Box sx={{ mt: 5, display: "flex", flexDirection: "column", gap: 1 }}>
          <Box data-color-mode={mode} sx={{ maxWidth: "100%" }}>
            <MDEditor
